...
class PortfolioEnv(gym.Env):
  metadata = {'render.modes': ['human']}
    ...
    def __init__(self,  edge=0.6, max_wealth=200000, max_rounds=300):
      ...
      # gold, SPY(S&P500), QQQ(NASDAQ),Arbitrage, inverse?
      #할 수 있는 경우의 수가 Box( ) 안에 들어감!
      # increments
      self.observation_space = spaces.Tuple((
          spaces.Box(0, max_wealth, shape=[1], dtype=np.float32),  # (w,b) box는 실수형, discrete는 이산형 범위!
          #spaces.Discrete(max_rounds + 1),
          spaces.Box(n일 치 T10Y2Y),
          spaces.Box(n일 치 거시경제지표)))
